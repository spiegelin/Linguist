...
const findOrCreate = async (profile) => {
    ...
    let query = `SELECT * FROM users WHERE facebook_id = '${profile.id}'`;
    ...
    if (res.rows.length === 0) {
        query = `INSERT INTO users (facebook_id, first_name, last_name, email) VALUES ('${profile.id}', '${profile.name.givenName}', '${profile.name.familyName}', '${profile.emails[0].value}') RETURNING *`;
